...
class reporte_pdf_numeros extends Controller
{
    //
    public function generarPdf(Request $request)
    {
        ...
        try{
            if($area == 'COORDINACIÓN'){
                $datos = RegistroNumeros::whereRaw(
                    'str_to_date(fecha, "%d/%m/%Y") BETWEEN str_to_date(?, "%d/%m/%Y") AND str_to_date(?, "%d/%m/%Y") order by fecha',
                    [$fecha_inicial, $fecha_final]
                )->get();
            }else {
                $datos = NumerosAreas::whereRaw(
                    'id_area = ? and str_to_date(fecha, "%d/%m/%Y") BETWEEN str_to_date(?, "%d/%m/%Y") AND str_to_date(?, "%d/%m/%Y") order by fecha',
                    [$area,$fecha_inicial, $fecha_final]
                )->get();
            }
        ...
        $pdf = FacadePdf::loadView('pdf.reporte_pdf_numeros', compact('datos','fecha_inicial','fecha_final'));
        $pdf->setPaper('letter', 'landscape');  // Para orientación horizontal
        // Devolver el PDF como respuesta
        return $pdf->download('Reporte de numeros entregados.pdf');
